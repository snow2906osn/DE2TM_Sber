...
select
       ...
       to_char(sum(case when cte.diff_m=1 then cte.income_sum_amt else 0 end),'99G999G990D99') SALARY_2M_AMT, /* !!! наименование на основании “«, но по идее SALARY_1M_AMT*/
       -- —умма транзакций за второй мес€ц до отчетной даты
       to_char(sum(case when cte.diff_m=2 then cte.income_sum_amt else 0 end),'99G999G990D99') SALARY_1M_AMT, /* !!! наименование на основании “«, но по идее SALARY_2M_AMT*/
       -- —умма транзакций за третий мес€ц до отчетной даты
       to_char(sum(case when cte.diff_m=3 then cte.income_sum_amt else 0 end),'99G999G990D99') SALARY_3M_AMT,
       -- —умма транзакций за четвертый мес€ц до отчетной даты
       to_char(sum(case when cte.diff_m=4 then cte.income_sum_amt else 0 end),'99G999G990D99') SALARY_4M_AMT,
       -- —умма транзакций за п€тый мес€ц до отчетной даты
       to_char(sum(case when cte.diff_m=5 then cte.income_sum_amt else 0 end),'99G999G990D99') SALARY_5M_AMT,
       -- —умма транзакций за шестой мес€ц до отчетной даты
       to_char(sum(case when cte.diff_m=6 then cte.income_sum_amt else 0 end),'99G999G990D99') SALARY_6M_AMT,
...
from (select it.client_id,
      ...
      from
           ...
           and (di.income_nm='«арплатное начисление' and
                to_date('2021-12-01','YYYY-MM-DD')>to_date(it.income_month,'YYYY-MM'))) cte
